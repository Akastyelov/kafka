...
Client.prototype.init = function () {
    ...
    self.initialBrokers = self.options.connectionString.split(',').map(function (hostStr) {
        ...
        if (self.options.sslOptions) {
          config.sslOptions = self.options.sslOptions;
          //To prevent error if server's sertificate has a wrong CN
          if (self.options.sslOptions.ignoreCertCN) {
            config.sslOptions.checkServerIdentity = function (host, cert) {
              return undefined;
            }
          }
        }
        ...
        return self.metadataRequest().then(function (response) {
            ...
            _.each(response.broker, function (broker) {
                ...
                self.brokerConnections[broker.nodeId] = connection || new Connection({
                    host: broker.host,
                    port: broker.port,
                    sslOptions: self.options.sslOptions
                });
            });
